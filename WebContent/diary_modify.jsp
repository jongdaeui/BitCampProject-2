...
<%	
	...
	System.out.println("diary_modify.jsp - cPage : " + cPage);
...
#title_content, #write_content { /* 입력창 */
	width: 510px;
	margin: -15px;
	border: none;
...
#modify_btn { /* 수정버튼(input button타입) */
	text-decoration: none;
	color: orange;
	margin-left: 10px;
	margin-bottom: 0;
...
function list_go() {
	location.href = "DiaryListController";		
}	
		...
		<div id="right1" class="box1">
			<div id="right2" class="box2">
				...
				<div id="right3" class="box3">
					...
					<form action="DiaryModifyOkController?cPage=${cPage }" method="post" >	
						<div id="title2" class="diary">
							<input type="text" id="title_content" name="title" title="제목" value="${dvo.title }" maxlength="30">				
						</div>					
						<div id="content" class="diary">
							<textarea id="write_content" name="content" rows="17" cols="50" title="내용" >${dvo.content }</textarea>
						</div>
						<div id="btnDiv">
							<input type="button" value="수정" id="modify_btn" onclick="sendData(this.form)">
