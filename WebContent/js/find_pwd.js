...
function find_send() {
	if (validate()) {
		...
		$.ajax({
			...
			type: "post",
			data: { id: fr.id.value, name: fr.name.value, phone: fr.phone.value, birth: fr.birth.value, type: "findPwd" },
			dataType: "text",
			success: function(data) {
				if (data != "null") {
					alert(fr.name.value + " 고객님의 정보가 확인되어 재설정 페이지로 이동하겠습니다.");
					location.href = "accountController?type=moveResetPwd&id=" + fr.id.value;
				} else if(data == "null") {
					alert("일치하는 정보가 없습니다.\n확인 후 다시 진행해주세요.");
				}
			},
